...
public class ShopOfficeSearch extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.shop_office_search); 
	...
	public void init() {
		Util.initTop(this, "搜索", Integer.MIN_VALUE, new OnClickListener() {
			@Override
			public void onClick(View v) {
				ShopOfficeSearch.this.finish();
			}
		});
	}
	...
	private void getOffice(){
		...
		Util.asynTask(this, "", new IAsynTask() {
			...
			public void updateUI(Serializable runData) {
				if(runData!=null){
					...
					if(list!=null&&list.size()>0){
						if (adapter == null) {
							adapter = new ItemAdapter(ShopOfficeSearch.this);
			...
			public Serializable run() {
				Web web=new Web(Web.officeUrl,Web.GetOfficeListPage,"loginUserId="+UserData.getUser().getUserId()+"&KeyWord1="+KeyWord1+"&sLevel="+sLevel+"&curpage="+(page++)+"&PageSize=20"+"&order_="+order);
	...
	public class ItemAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(!Util.isNull(s.getVipMcount())){
				h.vip_number.setText(Util.spanGreenWithString("VIP 数量：", s.getVipMcount()));
			}else{
				h.vip_number.setText(Util.spanGreenWithString("VIP 数量：", ""));
			}
			...
			if(!Util.isNull(s.getShopMerCount())){
				h.kj_number.setText(Util.spanGreenWithString("空间数量：", s.getShopMerCount()));
			}else{
			...
			if(!Util.isNull(s.getLogo())){
				...
				bmUtils.display(img, url, new BitmapLoadCallBack<View>() {
					...
					public void onLoadFailed(View arg0, String arg1,
							Drawable arg2) {
						Resources r = ShopOfficeSearch.this.getResources();
	...
	private void store(final String officesId,final ShopOfficeListModel m){
		if(UserData.getUser()!=null){
			...
			Util.asynTask(ShopOfficeSearch.this,"",new IAsynTask() {
				@Override
				public void updateUI(Serializable runData) {
					if(runData!=null){
						if("success".equals(runData+"")){
							Toast.makeText(ShopOfficeSearch.this, "收藏成功", Toast.LENGTH_LONG).show();
							m.setSc("1");
							adapter.notifyDataSetChanged();
						}
					}else{
		...
		if(UserData.getUser()!=null){
			...
			Util.asynTask(ShopOfficeSearch.this,"",new IAsynTask() {
				...
				public void updateUI(Serializable runData) {
					if(runData!=null){
						if("success".equals(runData+"")){
							Toast.makeText(ShopOfficeSearch.this, "取消收藏成功", Toast.LENGTH_LONG).show();
							m.setSc("0");
							adapter.notifyDataSetChanged();
						}
					}else{
...
//		Util.drawLayoutDropShadow(ShopOfficeList.this,Util.dpToPx(ShopOfficeList.this, 165), Util.dpToPx(ShopOfficeList.this, 130),pop_layout);

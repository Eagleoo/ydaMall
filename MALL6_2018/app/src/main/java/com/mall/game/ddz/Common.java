...
public class Common {
					...
					for (int l = 0, len4 = model.a123.size(); l <= len4; l++)
						for (int m = 0, len5 = model.a112233.size(); m <= len5; m++)
							for (int n = 0, len6 = model.a111222.size(); n <= len6; n++) {
								...
								// 我承认这个循环有点长..相信你的CPU
								bestModel = Common.getBestModel(newlist, model,
										new int[] { i, j, k, l, m, n });
								// 加上独立的牌
								...
								if (Common.getTimes(bestModel) < time) {
									...
									myModel = bestModel;
								} else if (Common.getTimes(bestModel) == time
										&& Common.getCountValues(bestModel) > value) {
	...
	public static void showCards2(Model model, List<Card> to, List<Card> from,
		...
		case c1:
			...
			for (int len = model.a1.size(), i = len - 1; i >= 0; i--) {
				if ((!isLessFive())
						&& Common.getValueByName(model.a1.get(i)) >= 16)
					break;
				if (Common.getValueByName(model.a1.get(i)) > Common
						.getValue(oppo.get(0))) {
			...
			if (Common.isLessFive()) {
				for (int i = 0, leni = from.size(); i < leni; i++) {
					if (Common.getValue(from.get(i)) > Common.getValue(oppo
							.get(0))) {
						to.add(from.get(i));
						break;
					}
		...
		case c2:
			...
			for (int len = model.a2.size(), i = len - 1; i >= 0; i--) {
				if ((!isLessFive())
						&& Common.getValueByName(model.a2.get(i)) >= 15)
					break;
				if (Common.getValueByName(model.a2.get(i)) > Common
						.getValue(oppo.get(0))) {
			...
			if (Common.isLessFive() && model.a3.size() > 0) {
				...
				for (int len = model.a3.size(), i = len - 1; i >= 0; i--) {
					...
					if (Common.getValueByName(model.a3.get(i)) > Common
							.getValue(oppo.get(0))) {
						List<Card> t = Common.getCardsByName(from,
								model.a3.get(i));
						to.add(t.get(0));
						to.add(t.get(1));
						return;
		...
		case c3:
			...
			for (int len = model.a3.size(), i = len - 1; i >= 0; i--) {
				if ((!isLessFive())
						&& Common.getValueByName(model.a3.get(i)) >= 15)
					break;
				if (Common.getValueByName(model.a3.get(i)) > Common
						.getValue(oppo.get(0))) {
		...
		case c411:
			...
			if (!(len1 < 1 || len2 < 2)) {
				for (int len = len1, i = len - 1; i >= 0; i--) {
					if (Common.getValueByName(model.a4.get(i)) > Common
							.getValue(oppo.get(0))) {
		...
		case c123:
			...
			for (int len = model.a123.size(), i = len - 1; i >= 0; i--) {
				...
				if (s.length == oppo.size()
						&& Common.getValueByName(model.a123.get(i)) > Common
								.getValue(oppo.get(0))) {
		...
		case c1122:
			...
			for (int len = model.a112233.size(), i = len - 1; i >= 0; i--) {
				...
				if (s.length == oppo.size()
						&& Common.getValueByName(model.a112233.get(i)) > Common
								.getValue(oppo.get(0))) {
			...
			if (!(len1 < 1 || len2 < 2)) {
				for (int i = len1 - 1; i >= 0; i--) {
					...
					if ((s.length / 3 <= len2)
							...
							&& Common.getValueByName(model.a111222.get(i)) > Common
									.getValue(oppo.get(0))) {
	...
	public static void showCards(Model model, List<Card> to, List<Card> from) {
		...
		// 有对子出对子 (除开3带，飞机)
				...
				&& Common.getValueByName(model.a2.get(model.a2.size() - 1)) < 15) {
			list.add(model.a2.get(model.a2.size() - 1));
		}
		// 有3带就出3带，没有就出光3
		else if (model.a3.size() > 0
				&& Common.getValueByName(model.a3.get(model.a3.size() - 1)) < 15) {
			// 3带单,且非关键时刻不能带王，2
			if (model.a1.size() > 0) {
				list.add(model.a1.get(model.a1.size() - 1));
			}// 3带对
			...
			list.add(model.a1.get(model.a1.size() - 1));
				...
				.size())) {
			list.add(model.a2.get(model.a2.size() - 1));
		} else if (Common.getValueByName(model.a3.get(0)) < 15
				&& model.a3.size() > 0) {
			// 3带单,且非关键时刻不能带王，2
			if (model.a1.size() > 0) {
				list.add(model.a1.get(model.a1.size() - 1));
		...
		if (flag == 1) {
			for (int i = 0, len = list.size(); i < len; i++) {
				...
				if (card.clicked)
					y -= card.height / 3;
				if(list.size()==20){  
					card.setLocation((int)(view.screen_width*0.44) - (len / 2 - i)* card.width/ 4, y);   
				}else{
					card.setLocation(view.screen_width*3/7 - (len / 2 - i)* card.width/ 4, y);   
				}
			}
		}  
		if (flag == 0) {
			for (int i = 0, len = list.size(); i < len; i++) {
				Card card = list.get(i);
				card.setLocation(view.cardWidth/2, view.cardHeight / 2 + i* view.cardHeight / 21);
			}
		}
		if (flag == 2) {
			for (int i = 0, len = list.size(); i < len; i++) {
				Card card = list.get(i);
				card.setLocation(view.screen_width - view.cardWidth *3/2,
						view.cardHeight / 2 + i* view.cardHeight / 21);
			}
		}
	}
	...
	public static void getBoomb(List<Card> list, Model model) {
		 ...
		 * != 5) { //del.add(list.get(0)); model.a1.add(list.get(0).name); }
		 */
		// list.removeAll(del);
		// 一般的炸弹
		for (int i = 0, len = list.size(); i < len; i++) {
	...
	public static Model getBestModel(List<Card> list2, Model oldModel, int[] n) {
		...
		for (int i = 0; i < n[3]; i++) {
			if (Common.isExists(list2, oldModel.a123.get(i))) {
				...
				list2.removeAll(Common.getCardsByName(list2,
						oldModel.a123.get(i)));
			}
		}
		// 双顺
	...
	public static void checkModel(List<Card> list, Model model1,
		...
		for (int i = 0, len = model1.a112233.size(); i < len; i++) {
			...
			if (flag == 0) {
				...
				list.removeAll(Common.getCardsByName(list,
						model1.a112233.get(i)));
			}
		}
	...
	public static Boolean isLessFive() {
		if (!Common.isFriend()
				&& view.playerList[Common.oppoerFlag].size() <= 5) {
			return true;
		}
		return false;
	}
	...
	public static Boolean isSingleOpper() {
		if (!Common.isFriend()
				&& view.playerList[Common.oppoerFlag].size() == 1)
			return true;
		else
			return false;
	}

...
public class a_Jxck extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.setContentView(R.layout.a_jianxichangke);
        ...
        PayType.addPayTypeListener(this, R.id.pay_item_xj_line, twoRow, xj,
                rec, uppay, weixin, ali);
        Spanned html1 = Html.fromHtml("我已阅读并同意<font color=\"#49afef\">《见习创客合同》"
                + "</font>");
            ...
            public void updateUI(Serializable runData) {
                if (null != runData) {
                    ...
                    if (9 == levelId || 10 == levelId || 5 == levelId || isSite) {
                        ...
                        Util.show("该用户不能做为辅导老师！", a_Jxck.this);
    ...
    private void intComponent() {
        Util.initTop(this, "申请〔见习创客〕", Integer.MIN_VALUE, null);
        if (null != UserData.getUser()) {
            ...
            Util.asynTask(new IAsynTask() {
                ...
                public void updateUI(Serializable runData) {
                    if (null == runData) {
                        Util.show("获取账户余额失败，请充值！", a_Jxck.this);
                        return;
                    }
    ...
    public void submit(View v) {
        ...
        if (!Util.checkUserInfocomplete()){
            VoipDialog voipDialog = new
                    VoipDialog("根据政府相关规定，从事互联网业务，需要进行实名登记", a_Jxck.this, "立即登记", "稍后登记", new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Util.showIntent(a_Jxck.this,
        ...
        Util.asynTask(a_Jxck.this, "正在处理，请稍等...", new IAsynTask() {
            @Override
            public void updateUI(Serializable runData) {
                if (null == runData) {
                    Util.show("网络错误，请重试！", a_Jxck.this);
                    return;
                }
                if ("success".equals(runData + ""))
                    Util.showIntent("申请成功！", a_Jxck.this, ProxySiteFrame.class);
                    ...
                    if ("您的充值账户余额不足！".equals(msg))
                        Util.showIntent("充值账户余额不足，是否前去充值？", a_Jxck.this,
                                AccountRecharge.class);
                    else
                        Util.show(msg, a_Jxck.this);
                }
            }

            @Override
            public Serializable run() {
                Web web = new Web(Web.jxck, "userId="
                        + UserData.getUser().getUserId() + "&md5Pwd="
                        + UserData.getUser().getMd5Pwd() + "&twoPwd="
                        + new MD5().getMD5ofStr(twoPwd.getText().toString())
                        + "&payType=" + getPayType() + "&money=" + 300
    ...
    private void wxPay(final String orderId, final double money) {
        CustomProgressDialog pd = CustomProgressDialog.showProgressDialog(this,
                "微信支付中...");
        new MMPay(this, pd, money, orderId, "申请见习创客").pay();
    }

    private void aliPay(String orderId, double money) {
        new AliPayNet(this).pay(orderId, "申请见习创客", money, new AliPayCallBack() {
            @Override
            public void doSuccess(String aliResultCode) {
                Util.showIntent(a_Jxck.this, Lin_MainFrame.class);
                Util.show("见习创客申请成功！", a_Jxck.this);
            }

            @Override
            public void doFailure(String aliResultCode) {
                Util.show("申请失败，支付宝状态码" + aliResultCode, a_Jxck.this);
            }
        });
    }

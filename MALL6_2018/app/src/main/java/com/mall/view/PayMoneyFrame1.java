...
public class PayMoneyFrame1 extends Activity {

    @ViewInject(R.id.goodsprice)
    private TextView goodsprice; // 第一行的商品总价

    @ViewInject(R.id.car_pay_item_reccon_rec_ban)
    private TextView gowuquan;
    ...
    private void showmoney(View view) {
        ...
        gowuquan.setVisibility(View.INVISIBLE);
        switch (view.getId()) {
            ...
            case R.id.pay_item_zz1_line:
                ...
                break;
            case R.id.car_pay_item_reccon_rec_ban:
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.setContentView(R.layout.pay_money_frame1);
...
//		this.findViewById(R.id.pay_item_zz_ban).setVisibility(View.GONE);
        ...
        PayType.addPayTypeListener(
                new PayTypeClickCallback() {
                    ...
                    public void click(View v) {
                        ...
                        if (v.getId() == R.id.pay_item_sb_line
                            ...
                            Util.asynTask(PayMoneyFrame1.this, "正在获取您的帐户信息...",
                                    new IAsynTask() {
                                        @Override
                        ...
                        if (v.getId() == R.id.car_pay_item_reccon_line) {
                            User user = UserData.getUser();
                            Map<String, String> map = new HashMap<String, String>();
                            map.put("userId", user.getUserId());
                            map.put("md5Pwd", user.getMd5Pwd());
                ...
                }, this, R.id.car_pay_item_reccon_line, twoLine, ali, rec, uppay,
                weixin, sbv,
                reccon, zzline, xj, carpayline, hbzz);
...
//        PayType.getPoint3().setTag(null);
    ...
    public void initComponent() {
        Util.initTitle(this, "订单支付", new OnClickListener() {
            ...
            public void onClick(View v) {
                Util.showIntent(PayMoneyFrame1.this, OrderFrame.class);
                finish();
            }
        });
        ...
        Util.asynTask(this, "正在支付，请稍等...", new IAsynTask() {
            ...
            public void updateUI(Serializable runData) {
                if ("success".equals(runData)) {
                    Util.showIntent(PayMoneyFrame1.this, OrderResultActivity.class, new String[]{"result", "number_order", "date_order"}, new String[]{"0", Util.order.getOrder().get(0).getOrderId(), Util.order.getOrder().get(0).getDate()});
                    ...
                    if ("wx".equals(arr[0])) {
                        CustomProgressDialog pd = CustomProgressDialog
                                .showProgressDialog(PayMoneyFrame1.this,
                                        "微信支付中...");
                        new MMPay(PayMoneyFrame1.this, pd, Double
                                .parseDouble(arr[2]), arr[1], "商品订单支付").pay();
                    } else {
                        Util.show("创建微信订单失败！", PayMoneyFrame1.this);
                    }
                } else if ("ali".equals(selectedPayType)) {
                    Log.i("tag", "支付" + runData);
                    String[] arr = (runData + "").split(":");
                    new AliPayNet(PayMoneyFrame1.this).pay(arr[1], "商品订单支付",
                            Double.parseDouble(arr[2]), new AliPayCallBack() {
                                ...
                                public void doFailure(String aliResultCode) {
                                    Util.showIntent(PayMoneyFrame1.this, OrderResultActivity.class, new String[]{"result", "number_order", "date_order"}, new String[]{"1", Util.order.getOrder().get(0).getOrderId(), Util.order.getOrder().get(0).getDate()});
                                }
                            });
                } else {
                    if (Util.isNull(runData)) {
                        ...
                        Util.show(runData + "", PayMoneyFrame1.this);
                    }
                }
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if ("success".equalsIgnoreCase(str)) {
            Util.showIntent("支付成功。点击确认查看订单状态。", PayMoneyFrame1.this,
                    OrderFrame.class);
        } else {
        ...
        erweima.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (Util.saveBitmapToSdCard(goodsimg, "代付二维码", bitmap)) {
                    ...
                    Toast.makeText(PayMoneyFrame1.this, "保存失败", Toast.LENGTH_SHORT).show();
                }
                return false;
            }
        });
        ...
        Util.asynTask(this, "发起代付…", new IAsynTask() {
            ...
            public void updateUI(Serializable runData) {
                ...
                if (runData != null) {
                    ...
                    if (map == null) {
                        Toast.makeText(PayMoneyFrame1.this,
                        ...
                        if (map.get("code") != null) {
                            if (map.get("code").equals("200")) {
                                ...
                                Toast.makeText(PayMoneyFrame1.this, "发起代付失败",
                                        Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(PayMoneyFrame1.this,

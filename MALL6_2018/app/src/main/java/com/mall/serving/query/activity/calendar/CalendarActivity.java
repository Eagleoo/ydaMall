...
import com.mall.serving.query.activity.calendar.CalendarInfo.ResultBean.DataBean;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		add_js.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				Intent intent = new Intent(CalendarActivity.this, AddOneNote.class);
...
//		}
	...
	private void initView() {
		...
		back=(TextViewBack) findViewById(R.id.top_left);
		...
		Accordingtime=(TextView) findViewById(R.id.Accordingtime);
		Accordingyear=(TextView) findViewById(R.id.Accordingyear);
...
//						asyncloadData(CalendarUtil.getDay(calSelected));
						Log.e("时间1",CalendarUtil.getDay(calSelected));
						String[] time=CalendarUtil.getDay(calSelected).split("-");
						String months=time[1];
						...
						Log.e("时间2",time[0]+"-"+months+"-"+day);
						getcalendarInfo(""+time[0]+"-"+months+"-"+day);
...
//					}
	...
	public void getcalendarInfo(final String times){
		...
		com.mall.serving.community.util.Util.asynTask(new IAsynTask() {
			...
			Gson gson=new Gson();
			CalendarInfo cInfo=gson.fromJson(runData.toString(), CalendarInfo.class);
			if (cInfo==null){
				Util.show("网络异常,请检查网络！", context);
				return;
			}

			String reason=cInfo.getReason()+"";
			if (reason.equals("Success")) {
				if (cInfo.getResult().getData()==null) {
					Toast.makeText(context, "未获取到数据，请稍后重试", Toast.LENGTH_SHORT).show();
					return;
				}
				
				DataBean daBean=cInfo.getResult().getData();
//				avoid =(TextView) findViewById(R.id.avoid);
//				suit =(TextView) findViewById(R.id.suit);
				avoid.setText(daBean.getAvoid());
				suit.setText(daBean.getSuit());
				Accordingtime.setText("农历"+daBean.getLunar());
				Accordingyear.setText(daBean.getLunarYear()+" ("+daBean.getAnimalsYear()+")");
			}else{
			...
			public Serializable run() {
				// TODO Auto-generated method stub
				Web web = new Web(
						"http://v.juhe.cn/calendar/day?date="+times+"&key="+"4662fa82849c4f28bb2c37aa1f0c66d5");
				return web.getPlan();
			}
		});
		...
		NewWebAPI.getNewInstance().getWebRequest("/Note.aspx?call=getImportantNotes",params, new WebRequestCallBack(){
			...
			public void success(Object result) {
				...
				if(200 != json.getIntValue("code")){
					Util.show(json.getString("message"), CalendarActivity.this);
					return ;
				}
				...
				adapter = new CaNoteAdapter(CalendarActivity.this, noteModels);
				listView.setAdapter(adapter);
				if (noteModels.size() == 0) {
					Toast.makeText(CalendarActivity.this, "当天无记事",

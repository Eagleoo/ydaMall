...
public class TrainTicketQueryActivity extends BaseActivity {
	...
	private String[] tt = new String[] { "全部", "高铁", "动车", "特快", "直达", "快速",
			"其他" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String city_from = SharedPreferencesUtils
				.getSharedPreferencesString("city_from");
		String city_to = SharedPreferencesUtils
				.getSharedPreferencesString("city_to");
		...
		String dayByDate = GetLastDateTime.getInstance().getDayByDate(
				new Date(), 0);
		String dayByDate1 = GetLastDateTime.getInstance().getDayByDate(
				new Date(), 1);
		c2.add(Calendar.DATE, 1);
		tv_time_from.setText(Util.formatDateTime("yyyy-MM-dd", "yyyy-MM-dd E",
				dayByDate));
	...
	private void setView() {
		rb_rg_1.setText("单程");
		rb_rg_2.setText("往返");
		top_center.setText("火车票查询");
		top_left.setVisibility(View.VISIBLE);
		tr_time_to.setVisibility(View.GONE);
	}

	@OnClick({ R.id.tv_city_from, R.id.tv_city_to, R.id.tv_time_from,
			R.id.tv_time_to, R.id.tv_type, R.id.tv_query })
	public void click(View v) {

		switch (v.getId()) {
		case R.id.tv_city_from:
			Intent intent = new Intent(this, TrainTicketCityQueryActivity.class);
		...
		case R.id.tv_query:
			...
			Util.showIntent(context, TrainTicketQueryListActivity.class,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == 100) {
			...
			if (requestCode == 101) {
				...
				SharedPreferencesUtils.setSharedPreferencesString("city_from",
						city);
			} else if (requestCode == 102) {
				tv_city_to.setText(city);
				SharedPreferencesUtils.setSharedPreferencesString("city_to",
						city);
			}
		}

...
public class Flake {
	...
	float rotation;
	float speed;
	public static float speedMin=100;
	float rotationSpeed;
	int width, height;
	Bitmap bitmap;
	...
	static Flake createFlake(float xRange, Bitmap originalBitmap) {
		...
		flake.y = 0 - (flake.height + (float) Math.random() * 300);

		// Each flake travels at 50-200 pixels per second
		flake.speed = speedMin + (float) Math.random() * speedMin;
